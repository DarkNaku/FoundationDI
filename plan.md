...
## 완료: AspectFillImage Fit 기준 위치

`AspectFillImage`의 Fit은 종횡비를 유지해 부모 안에 맞추므로 한 축에 여백이 남는데, `pivot`과
`anchoredPosition`이 중앙 고정이라 그 여백이 항상 양쪽에 균등하게 나뉘었다. 배경 이미지의 위쪽을
살리고 아래를 잘라내는 식의 배치를 할 수 없다.

9분할 기준 위치를 지정할 수 있게 한다. 앵커 구조(부모 스트레치)는 그대로 두고 `anchoredPosition`만
옮긴다 — Fit의 `sizeDelta`는 "부모보다 얼마나 작은가"라 음수이고 그 절댓값이 곧 여백이므로,
중앙(0.5)에서 벗어난 비율을 곱하면 이동량이 나온다. 꽉 찬 축은 `sizeDelta`가 0이라 자동으로
움직이지 않는다.

**Cover는 제외한다** — 여백이 아니라 넘침이라 몰아 줄 것이 없고, 중앙을 유지해야 잘림이 양쪽에
균등하게 일어난다. 기준 위치 타입은 유니티의 `TextAnchor`를 그대로 쓴다(`LayoutGroup.childAlignment`가
같은 개념으로 쓰는 public enum이라 자체 enum을 만들 이유가 없다). 기본값 `MiddleCenter`가 기존
동작과 동일해 이미 배치된 오브젝트는 영향을 받지 않는다.

- [x] Fit에서 세로 여백이 기준 위치의 반대쪽으로 몰린다
- [x] Fit에서 가로 여백이 기준 위치의 반대쪽으로 몰린다
- [x] Cover에서는 기준 위치를 무시하고 중앙에 둔다
- [x] 기준 위치의 기본값은 중앙이다

---

## 완료: 2D 사운드 같은 프레임 중복 재생 방지

`Sound.Play()`는 논루프 재생에서 이전 재생을 멈추지 않고 매번 새 소스를 받아 겹쳐 재생한다.
2D(`spatialBlend = 0`)는 거리 감쇠가 없어 볼륨이 항상 1.0이고 두 소스의 위치·패닝이 동일하므로,
같은 프레임에 같은 클립이 겹치면 **샘플 단위로 완전히 일치**해 진폭이 정확히 2배가 된다.
유니티 믹서는 float이라 그 단계에서는 안 깨지지만 출력 단계에서 [-1,1]로 클램프되며 왜곡된다.
UI 클릭음처럼 노멀라이즈된 클립이 특히 위험하다.

같은 프레임에 같은 클립이 이미 재생을 시작했다면 스킵한다. 판정은 **클립 단위로 전역**이다
(서로 다른 버튼이 같은 클릭음을 같은 프레임에 내는 것도 같은 문제라 `Sound` 인스턴스별로는
막을 수 없다). **루프는 제외한다** — 루프의 `Play()`는 이전 재생을 `Stop()`하고 교체하므로
겹치지 않고, 여기서 스킵하면 정지만 되고 새 재생이 막혀 소리가 사라진다.

`Time.frameCount`는 EditMode에서 테스트가 제어할 수 없어 `ISoundFrameClock` seam으로 감싼다.

- [x] 2D 사운드는 같은 프레임에 같은 클립을 두 번 재생하지 않는다
- [x] 프레임이 바뀌면 같은 클립을 다시 재생한다
- [x] 2D라도 클립이 다르면 같은 프레임에 함께 재생한다
- [x] 3D 사운드는 같은 프레임에 같은 클립을 겹쳐 재생한다
- [x] 서로 다른 Sound 인스턴스도 같은 클립이면 같은 프레임에 스킵된다
- [x] 루프 2D 사운드는 같은 프레임 재시작이 막히지 않는다
