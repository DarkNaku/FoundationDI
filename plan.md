...
## 활성 계획: TutorialManager — 조건 기반 튜토리얼 진행 엔진
...
- [ ] 타깃참조가 비어있으면 IsEmpty가 참이다
- [ ] 키만 채우면 HasKey가 참이고 비어있지 않다
- [ ] 공백문자열 키는 키가 없는 것으로 본다
- [ ] 직접참조를 채우면 비어있지 않고 키는 없다
- [ ] 직접참조가 파괴되면 다시 비어있는 것으로 본다
- [ ] 직접참조가 키보다 우선한다
- [ ] 저장한적 없는 시퀀스는 NotStarted다
- [ ] 상태를 저장하면 새 인스턴스에서도 읽힌다
- [ ] 시퀀스마다 상태가 독립적이다
- [ ] 스텝인덱스를 저장하면 새 인스턴스에서도 읽힌다
- [ ] AllSkipped는 기본이 거짓이고 저장하면 유지된다
- [ ] Clear는 상태와 스텝인덱스와 AllSkipped를 모두 지운다
- [ ] 저장키가 다르면 진행도가 섞이지 않는다
- [ ] 가짜트리거는 Arm되기 전에 발동해도 아무일이 없다
- [ ] 가짜트리거는 Arm 후 Fire하면 콜백을 부른다
- [ ] 가짜트리거는 Disarm되면 Fire해도 콜백을 부르지 않는다
- [ ] 가짜시계는 대기를 즉시 끝낸다
- [ ] 가짜모듈은 Show와 Hide 횟수를 센다
- [ ] 가짜레지스트리는 등록된 타깃을 즉시 돌려준다
- [ ] 타깃핸들은 대상이 바뀌면 Changed를 쏜다
- [ ] 타깃핸들은 같은 대상으로 다시 설정하면 Changed를 쏘지 않는다
- [ ] 타깃핸들은 대상이 파괴되면 Current가 null이다
- [ ] 타깃핸들은 Dispose 후 Changed를 쏘지 않는다
- [ ] 스텝은 트리거가 없으면 Auto로 채운다
- [ ] 스텝은 음수 지연을 0으로 보정한다
- [ ] 스텝은 null 모듈을 걸러낸다
- [ ] 시퀀스는 스텝이 없으면 빈 목록을 갖는다
- [ ] 시퀀스는 null 스텝을 걸러낸다
- [ ] 시퀀스는 음수 타임아웃을 0으로 보정한다
- [ ] 트리거어웨이터는 발동하면 완료된다
- [ ] 트리거어웨이터는 취소되면 Disarm하고 취소예외를 던진다
- [ ] 트리거어웨이터는 두번 발동해도 한번만 완료된다
- [ ] 시작트리거가 발동해야 시퀀스가 시작된다
- [ ] 스텝이 시작트리거-모듈Show-종료트리거-모듈Hide 순서로 진행된다
- [ ] 여러 스텝이 순서대로 진행된다
- [ ] 시퀀스가 완료되면 Completed로 기록되고 이벤트가 발행된다
- [ ] 완료된 시퀀스는 등록해도 트리거를 arm하지 않는다
- [ ] AllSkipped면 어떤 시퀀스도 arm하지 않는다
- [ ] 중복 시퀀스ID는 무시된다
- [ ] Unregister하면 트리거가 Disarm된다
- [ ] Dispose하면 대기중인 트리거가 모두 Disarm된다
- [ ] Auto트리거는 Arm 즉시 발동한다
- [ ] Manual트리거는 같은 ID로 Fire해야 발동한다
- [ ] Manual트리거는 Disarm되면 발동하지 않는다
- [ ] Message트리거는 Match를 통과한 메시지에만 발동한다
- [ ] Message트리거는 Match를 오버라이드하지 않으면 모든 메시지에 발동한다
- [ ] Message트리거는 Disarm하면 구독이 해제된다
- [ ] Message트리거는 한번 발동한 뒤 다시 발동하지 않는다
- [ ] ButtonClick트리거는 타깃 버튼을 누르면 발동한다
- [ ] ButtonClick트리거는 Disarm하면 리스너가 제거된다
- [ ] ButtonClick트리거는 타깃이 없으면 발동하지 않고 예외도 없다
- [ ] 직접참조는 등록없이 해석된다
- [ ] 등록되지 않은 키는 해석되지 않는다
- [ ] 등록한 키가 해석된다
- [ ] 같은 키를 두번 등록하면 마지막 등록이 이긴다
- [ ] 마지막 등록을 해제하면 이전 등록으로 돌아간다
- [ ] 이미 등록된 타깃은 즉시 해석된다
- [ ] 나중에 등록되는 타깃을 기다린다
- [ ] 해석된 핸들은 타깃이 해제되면 null이 된다
- [ ] 해석된 핸들은 타깃이 다시 등록되면 복귀한다
- [ ] 핸들을 Dispose하면 등록해도 영향받지 않는다
- [ ] 타임아웃이 지나면 null을 돌려준다
- [ ] 빈 참조는 null 대상의 핸들을 즉시 돌려준다
- [ ] 실행중 다른 시퀀스가 발동하면 대기열에 들어간다
- [ ] 대기열은 Order 오름차순으로 실행된다
- [ ] 기본 재개모드는 시퀀스 처음부터 시작한다
- [ ] ResumeFromStep이면 저장된 스텝부터 시작한다
- [ ] Running 상태여도 시작트리거를 기다린다
- [ ] 스텝 지연이 시계를 통해 대기된다
- [ ] 모듈이 예외를 던져도 다음 모듈과 스텝이 진행된다
- [ ] 타깃을 못찾으면 시퀀스가 중단되고 NotStarted로 되돌아간다
- [ ] Skip은 현재 시퀀스만 완료처리한다
- [ ] SkipAll은 AllSkipped를 세우고 모든 트리거를 Disarm한다
- [ ] Dispose하면 진행중인 시퀀스가 취소되고 완료로 기록되지 않는다
- [ ] 등록하면 ITutorialManager를 해결할 수 있다
- [ ] 등록하면 타깃 레지스트리도 함께 해결된다
- [ ] ITutorialManager는 싱글톤이다
- [ ] 저장소를 직접 주입하면 그것이 쓰인다
- [ ] 타깃이 null이면 실패한다
- [ ] RectTransform은 코너로 rect를 만든다
- [ ] 렌더러가 없는 일반 Transform은 점 rect를 만든다
- [ ] 렌더러가 있으면 바운즈로 rect를 만든다
- [ ] 카메라가 없으면 일반 Transform은 실패한다
