...
- Modify: `Assets/FoundationDI/package.json` (0.8.0 → 0.8.1)
- Modify: `CLAUDE.md`
- Modify: `plan.md`
...
`Assets/FoundationDI/package.json`의 `"version": "0.8.0"`을 `"version": "0.8.1"`으로 바꾼다.

패치 증가다. 새 컴포넌트가 추가되지만 `SoundButton` 삭제 외에 기존 API를 깨지 않으므로 마이너를 올리지 않는다.
...
git add -A
git commit -m "$(cat <<'MSG'
[STRUCTURAL] SoundButton을 UIButton으로 대체하고 0.8.1로 올린다
