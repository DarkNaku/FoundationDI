...
> **정정 (최종 리뷰, 2026-08-30 브랜치 전체 리뷰)**
>
> 이 (c)의 전제가 결정 3 이후로 깨졌다. 이 재구성 경로는 `Root` getter가 실제로 평가되어야 도달하는데, `Root`는 `Pool` getter의 지연 초기화(`_pool ??= new PoolManager(..., Root.GO.transform)`)에서만 평가된다. 결정 3이 삭제한 씬 전환용 `ClearContent()`가 바로 `_pool`을 매 씬 전환마다 null로 되돌려 이 평가를 강제하던 유일한 경로였다. `Dispose()`만 남은 지금은 한 번이라도 표시가 있었던 UINavigator라면 `_pool`이 이미 non-null로 굳어 있어, 캔버스가 파괴돼도 `Pool` getter는 `Root`를 다시 건드리지 않는다.
>
> 그 결과 이 창에서 실제로 벌어지는 일은 "다음 씬에 캔버스 재구성"이 아니라 **호출 실패**다: `Pool.Get(key)`가 파괴된 풀 아이템을 꺼내 `MissingReferenceException`을 던지거나(흔한 경로), 새 View를 인스턴스화하되 이미 파괴된 풀 루트에 `SetParent`하면서 조용히 다음 씬 최상위로 떨어뜨린다. 두 경우 모두 `OperationQueue`가 예외를 로그로 남기고 삼키므로, 화면에는 아무 UI도 나타나지 않는다(고아 캔버스가 아니라 고아 View 또는 아무것도 없는 상태에 더 가깝다). (c)가 지키려던 "복구 동작"은 이제 이 창에서 안정적으로 재현되지 않는다.
>
> 이 재구성 경로를 실제로 되살리는 것(예: `Pool`이 fake-null 루트를 감지하게 하는 것)은 그 자체로 행동 변경이며 별도 TDD 사이클이 필요하다 — 이 정정은 문서만 바로잡을 뿐, 코드는 바꾸지 않는다.
