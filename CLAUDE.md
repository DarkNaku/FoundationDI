...
- **SoundService** (`Services/SoundService/`): 태그 기반 오디오 시스템.
  ...
  - **논루프 `Sound.Play()`는 이전 재생을 멈추지 않고 겹쳐 재생한다** — 매번 `GetSource()`로 새 소스를 받고 `_source`가 마지막 것만 가리키므로 앞선 재생은 고아가 된다(`Stop()`이 마지막 하나만 멈춘다). 그래서 **2D는 같은 프레임에 같은 클립을 두 번 재생하지 않는다**: 2D는 `spatialBlend = 0`이라 감쇠·패닝이 없어 두 신호가 샘플 단위로 일치하고 진폭이 정확히 2배가 되어 출력에서 클램프되며 왜곡된다. 판정은 `ISoundEngine.TryReserveClipThisFrame`이 **클립 단위로 전역** 수행한다(서로 다른 빌더가 같은 클립을 내는 경우도 막아야 한다). **3D와 루프는 제외** — 3D는 그대로 겹치지 않고, 루프는 `Play()`가 이미 이전 재생을 `Stop()`한 뒤라 여기서 막으면 소리가 통째로 사라진다.
  - 프레임 판정은 `ISoundFrameClock` seam이다 — `Time.frameCount`는 EditMode에서 테스트가 제어할 수 없다(에디터 틱에 따라 임의로 증가).
