...
- **전환 직렬화** — `ShowQueue`로 모든 전환을 순차 처리(race 제거)
...
{
    protected internal override void OnShow() { /* 표시 직후 */ }
}
```
...
| `OnShown(Action<...> cb)` | Shown 라이프사이클에 콜백 등록 |
...
- `Page/Popup/Overlay<T>()`는 인스턴스를 **즉시 동기 반환**하고 실제 표시는 `ShowQueue`에 enqueue된다. 따라서 같은 프레임에 빌더 체인(`.With/.OnShown/.WithTransition`)을 동기로 구성할 수 있다.
- 모든 Show/Hide 전환은 `ShowQueue`로 **순차 직렬화**되어 동시 전환의 race를 방지한다.
