...
`RegisterUIService` 호출 **전에 `IResourceService`가 등록**되어 있어야 합니다(프리팹 로드를 위임).
...
Presenter의 `Hide()`로 숨깁니다. 숨겨진 인스턴스는 캐시에 보관되어 다음 표시 때 재사용되며, 실제 파괴는 `UIService.Dispose()`(컨테이너 수명 종료) 시 일괄 처리됩니다.
...
`UIService`는 `IUIService`, `IDisposable`을 구현하며 `RegisterUIService`로 등록한다.
...
명령: `void Hide()` (숨김 + 캐시 보관). 개별 파괴는 제공하지 않으며, 정리는 `UIService.Dispose()`에서 일괄 수행된다.
...
- `void RegisterUIService(this IContainerBuilder builder, UIServiceSettings settings)` — UIService 등록 확장.
  **전제: 호출 전에 `IResourceService`가 등록되어 있어야 한다.**
- `UIServiceSettings` (ScriptableObject) — `ReferenceResolution`(기본 1920×1080) 제공. (공통 기본 트랜지션은 제거됨 — 트랜지션은 각 View에 부착한 컴포넌트로 지정한다.)

### Canvas / CanvasScaler

UIService는 루트 Canvas(`[UIService]`, ScreenSpaceOverlay)에 `CanvasScaler`를 **Scale With Screen Size + Screen Match Mode = Expand**로 구성하고, 기준 해상도로 `UIServiceSettings.ReferenceResolution`을 사용한다(설정이 없거나 0 이하면 1920×1080으로 폴백). 즉 해상도 대응은 UIService가 자동 처리하므로 View 프리팹에 별도 `CanvasScaler`를 둘 필요가 없다.
...
- 개별 파괴 API는 없다. 캐시·활성 인스턴스의 정리(파괴)는 `UIService.Dispose()`에서만 일괄 수행된다.
...
- `UIService.Dispose()`는 진행 중 큐를 취소하고, 활성/캐시된 모든 Presenter에 `OnDestroyElement` + `Destroyed` 이벤트를 발화한 뒤 캐시/컨트롤러를 비우고 UI 루트 Canvas를 파괴한다. 보통 DI 컨테이너가 수명을 관리한다.
...
- PlayMode (`Tests/Runtime`): `UIServiceFlowTests`(Page/Popup/Overlay 표시·재Show 활성화)·`UIViewTests`·`UIViewTransitionResolveTests`·`UIRootTests`·`FadeTransitionTests`·`SlideTransitionTests`·`ScaleTransitionTests`. 프리팹 로드는 `IResourceService`를 NSubstitute로 대체해 가짜 프리팹을 주입한다.
