...
- **실패(null) 로드는 참조 카운트를 올리지 않습니다.** `Load`/`LoadAsync`가 `null`을 반환하면 캐시·카운트에 반영되지 않으므로, 호출자는 실패에 대해 `Release` 짝을 맞출 필요가 없습니다(`null`이면 그냥 무시).
...
- **실패(null) 로드는 캐시하지 않습니다.** provider가 `null`을 반환하면 캐시에 좀비 엔트리를 남기지 않고 `provider.Release(key)`로 핸들을 정리하며, 다음 로드는 provider를 다시 호출합니다.
...
- `DefaultResourceService`(Resources)는 Addressables 같은 핸들 해제가 없어 메모리 반환이 제한적이다(`GameObject` 등 개별 언로드 불가, 확실한 회수는 `Resources.UnloadUnusedAssets()`).
