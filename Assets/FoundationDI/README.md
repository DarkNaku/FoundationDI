...
- **UI 컴포넌트** — uGUI `Button`을 상속한 `UIButton`(클릭 SFX + 햅틱), 상태별 이미지/텍스트 스왑 `UISwapButton`, 호버 확대·누름 축소 `UIScaleButton`(스케일을 자식에만 걸어 히트 영역이 변하지 않는다). 사운드·햅틱 서비스는 선택적이라 등록하지 않으면 그 기능만 조용히 꺼진다
