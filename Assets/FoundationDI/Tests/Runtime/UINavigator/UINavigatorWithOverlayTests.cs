...
public class UINavigatorWithOverlayTests
{
    ...
    [UnityTest]
    public IEnumerator WithOverlay는_Page와_오버레이를_함께_노출하고_Page_hide시_함께_숨긴다() => AwaitableTest.Run(async () =>
    {
        ...
        await AwaitableTest.WaitUntil(() => host.Shown, 3f);
        ...
        host.Hide();
        await AwaitableTest.WaitUntil(() => host.AfterHideCalled && ov.AfterHideCalled, 3f);
    ...
    [UnityTest]
    public IEnumerator persistent_오버레이는_페이지_전환시_동일_인스턴스로_연속유지된다() => AwaitableTest.Run(async () =>
    {
        var resource = Substitute.For<IResourceService>();
        resource.Load<GameObject>("UI/WithOvHost").Returns(_hostPrefab);
        resource.Load<GameObject>("UI/WithOvOverlay").Returns(_ovPrefab);
    ...
    [UnityTest]
    public IEnumerator 기본_오버레이는_페이지_전환시_새_인스턴스로_재생성된다() => AwaitableTest.Run(async () =>
    {
        var resource = Substitute.For<IResourceService>();
        resource.Load<GameObject>("UI/WithOvHost").Returns(_hostPrefab);
        resource.Load<GameObject>("UI/WithOvOverlay").Returns(_ovPrefab);
